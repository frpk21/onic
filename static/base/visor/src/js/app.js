          ...
          layer.on("click", function(evt){
            ...
            for (key in evt.layer.feature.properties){
              ...
              if (["FID", "OBJECTID", "OID", "id", "ID", "ObjectID"].indexOf(key) < 0){
                ...
                if (key !== null){
                  output += "<tr><td><b>" + key + "</td><td>" + val + "</td></tr>";
                }
              }
            }
            output += "</table>"
  ...
  for (var tocIdx = 0 ; tocIdx < tocCategories.length; tocIdx ++) {
    ...
    if (tocCategoryName in bootleaf.layerTOC === false) {
      ...
      if (tocCategory.exclusive) {
        ...
        bootleaf.layerTOC[tocCategoryName]["Ninguna"] = L.layerGroup();
      }
    }
    ...
    if (this.popup !== undefined){
      ...
      for (key in feature.properties){
        ...
        if (val !== null && val !== undefined && $.inArray(key, ["bbox", "geom", "gid"]) < 0) {
          ...
          if (key !== null) {
            popupContent += "<tr><td><b>" + key + "</td><td>"+ val + "</td></tr>";
          }
        }
      }
...
function configureQueryWidget(){
  ...
  if (bootleaf.queryTasks) {
    if (bootleaf.queryTasks.length > 0){
      ...
      resetSidebar("Filtros Temáticos", html);
      ...
      $("#btnReturnLayer").on('click', runReturnLayer);
...
function updateQueryFields(layerId){
  ...
  document.getElementById("btnReturnLayer").style.display = "none";
...
function updateQueryOperator(option){
  ...
  var operators = [
    {"value": "=", "alias": "Coincidencia exacta"},
    {"value": "starts with", "alias": "Empieza con"},
    {"value": "ends with", "alias": "Termina en"},
    {"value": "contains", "alias": "Contiene"}
  ];
  if (option !== undefined && option.dataset['fieldtype'] !== undefined){
    var fieldType = option.dataset["fieldtype"];
    if (fieldType === 'numeric'){
      operators = [
        {"value": "<", "alias": "Menor que"},
        {"value": "=", "alias": "Igual a"},
        {"value": ">", "alias": "Mayor que"}
      ];
    }
...
function runQueryWidget() {
  ...
  if (!bootleaf.visibleLayers.includes(layerId)){
    $.growl.warning({ message: "Habilite la capa de consulta"});
    return;
  }
  ...
  if (layer.type === 'agsDynamicLayer' || layer.type === 'agsFeatureLayer') {
    ...
    if($("#chkQueryWithinMapExtent").is(':checked')) {
      ...
      var geometry = {"xmin": bounds._southWest.lng,"ymin": bounds._southWest.lat,"xmax": bounds._northEast.lng,"ymax": bounds._northEast.lat,"spatialReference":{"wkid":4686}}
    ...
    $.ajax({
      ...
      success: function(data, textStatus, jqXHR) {
        try {
          handleQueryResults(data, jqXHR.layerConfig, outFields, layer);
        } catch(err) {
    ...
    $.ajax({
      ...
      success: function(data, jqXHR, response) {
        layer["where"] = queryData["CQL_FILTER"];
        console.log(queryData["CQL_FILTER"])
        console.log("There was a success running the query");
        try {
          handleQueryResults(data, response.layerConfig, outFields, layer);
        } catch(err) {
          handleQueryError("There was a problem running the query");
        }
      },
...
function runReturnLayer() {
  var layerId = $("#queryWidgetLayer option:selected").val()
  if (!bootleaf.visibleLayers.includes(layerId)){
    $.growl.warning({ message: "Habilite la capa a reestablecer"});
    return;
  }
  ...
  //let data = sessionStorage.getItem(layerId);
  ...
  document.getElementById("queryTableResults").style.display = "none";
  //bootleaf.map.flyTo(config.start.center, config.start.zoom);
}


function handleQueryResults(data, layerConfig, outFields, layer){
  ...
  if (data.features.length ===0 ){
    handleQueryError("No se encontrarón registros");
    runReturnLayer();
    return;
  }
  ...
  try{
    new $.fn.dataTable.Buttons( table, {
      buttons: [
          {
              extend: 'csvHtml5',
              text: 'Descargar resultados (archivo .CSV)',
              className: "btn-outline-dark"
          }]
      });
    table.buttons( 0, null ).containers().appendTo( $('#exportButtons') );
  } catch(err){
    console.log("There was a problem enabling DataTables for the Query Widget buttons", err)
  }

  document.getElementById("map").style.height = '70%';
  document.getElementById("queryTableResults").style.display = "block";

  wfsAjaxQuery(layer)
  document.getElementById("btnReturnLayer").style.display = "block";
...
function resetQueryOutputTable(){
  document.getElementById("map").style.height = '100%';
  document.getElementById("queryTableResults").style.display = "none";
  $("#queryResults").html('<span id="ajaxLoading"></span><table id="tblQueryResults" class="table table-condensed table-hover"></table><div id="exportButtons" style="margin-bottom: 70px;"></div>');
}
...
function updateFilterOperator(option){
  ...
  if (option !== undefined && option.dataset['fieldtype'] !== undefined){
    ...
    if (fieldType === 'numeric'){
      ...
      operators = [
        {"value": "<", "alias": "Antes de"},
        {"value": "=", "alias": "En la fecha"},
        {"value": ">", "alias": "Después de"}
      ]
    }
    // TODO: add other field types, eg date, etc
  }
...
function applyFilter() {
  ...
  for(var layerIdx=0; layerIdx < config.layers.length; layerIdx++){
    ...
    if (layerConfig.id === layerId){
      ...
      //console.log("LayerType", layerConfig.type)
      //console.log("filterTask", filterTask)

      // Build the filter where clause from the saved filter
      if (layerConfig.type === 'agsDynamicLayer' || layerConfig.type === 'agsFeatureLayer' || layerConfig.type === 'WFS') {
...
function configureCoordinatesTool(){
  ...
  resetSidebar("Marcar coordenadas","<p><span class='info'>Haga clic en el mapa para ver las coordenadas</span></p>");
  $("#sidebar").show("slow");
  switchOffTools();
...
function showHighlight(feature, zoom){
  ...
  if (zoom){
    ...
    for (field in feature.attributes){
      ...
      popup += "<tr><td><b>" + field + "</td><td>" + value + "</td><tr>";
    }
    popup += "</table>";
...
// Handle WFS layers
function fetchWFS(){
  console.log("número de capas ", bootleaf.labelLayers.length);
  ...
  //   //if (layer.layerConfig.cluster === undefined) {
  //     //var layerConfig = layer.layerConfig;
  //     //console.log("lastRun ", layer.lastRun);
  //     if (layer.lastRun) {
  ...
  //         //console.log("capa ", layer.layerConfig.cluster);
  //         //setTimeout(() => {  wfsAjax(layer); }, 1000);
  //         //wfsAjax(layer);
  //       }
  //     } else if (layer.tocState === "on"){
  //       //console.log("capa ", layer.layerConfig.cluster);
...
function wfsAjax(layer){
  console.log("Capa wfsAjax ", layer);

  const x = layer.deploy + 1;
  layer.deploy = x;

  if (layer.deploy > 1) {return;}
  ...
  console.log("data ", data);
  console.log("jsonpCallback ", layerConfig.callback || 'getJson');
  console.log("layerConfig ", layerConfig);

  //var url = "http://localhost:8080/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=isaa:Municipios&outputFormat=text/javascript&format_options=callback:getJson";
  ...
  $.ajax({
    ...
    success: function(data, jqXHR, response) {
      $("#loader").hide();
      console.log("WFS success with ", response.layerConfig.id);
      ...
      for (var x=0; x < bootleaf.wfsLayers.length; x++){
        ...
        if (layer.layerConfig.id === layerId){
          if (layer.layerConfig.cluster){
            ...
            //sessionStorage.setItem(layerId, tempLayer);
            //localStorage.setItem(layerId, JSON.stringify(tempLayer));
            //console.log("sessionStorage", tempLayer);
            layer.clearLayers();
            layer.addLayer(tempLayer);
            const y = 1;
            layer["deploy"] = y;
          } else {
            //sessionStorage.setItem("layerId", data);
            //localStorage.setItem(layerId, JSON.stringify(data));
            //console.log("dataToSessionStorage", data);
            //let dataAux = sessionStorage.getItem("layerId");
            //console.log("dataToSessionStorage", Object.values(dataAux));
            layer.clearLayers();
            layer.addData(data);
            layer.lastRun = null;
            const y = 1;
    ...
    //   var err = xhr.responseJSON.value;
    //   console.log(err);
    // }
...
function wfsAjaxQuery(layer){
  ...
  // are defined in the custom.js file (even if the functions are empty).
  var callback = "getJson";
  if (layer.callback !== undefined) {callback = layer.callback;}
  var data = {
    "service":"WFS",
    "version":"1.0.0",
    "request":"GetFeature",
    ...
    "format_options":"callback:" + callback,
    "CQL_FILTER": layer.where
  }

  data["outputFormat"] = "text/javascript";

  // Restrict the fields which are returned by the query. Ensure that the geometry field is included
  if (layer.fields && layer.fields.length && layer.fields.length > 0) {
    if ($.inArray(geomField, layer.fields) < 0) {
      layer.fields.push(geomField);
    }
    data.propertyName = layer.fields.join(",").replace(/, /g,",");
  }
  ...
  $.ajax({
    ...
    success: function(data, jqXHR, response) {
      ...
      var layerId = response.layerConfig.id;
      //console.log("layerId ", layerId);
      //console.log("bootleaf.wfsLayers", bootleaf.wfsLayers);
      for (var x=0; x < bootleaf.wfsLayers.length; x++){
        var layer = bootleaf.wfsLayers[x];
        if (layer.layerConfig.id === layerId){
          //localStorage.setItem('cachedData', JSON.stringify(layer));
          //console.log('Data fetched from server:', data);
          //console.log("layerId=layer.id", layer.id);
          if (layer.layerConfig.cluster){
            var tempLayer = new L.geoJson(null,layer.layerConfig);
            $(data.features).each(function(key, data) {
              tempLayer.addData(data);
            ...
            layer.clearLayers();
            //console.log("LayerClear ", layer);
            //sessionStorage.setItem("key", tempLayer);
            //localStorage.setItem('cachedData', JSON.stringify(tempLayer));
            //console.log("sessionStorage", tempLayer);
            layer.addLayer(tempLayer);
            ...
            //console.log("dataToLocalStorage", data);
            //const storedUserData = localStorage.getItem('cachedData')

            // if (storedUserData) {
            //   const userData = JSON.parse(storedUserData)
            //   console.log("dataFromLocalStorage", userData);
            // } else {
          ...
          if (layer.label !== undefined){
            createLabels(layer, data)
          }
        }
      }
      
      //data.features.forEach((element) => console.log(element.geometry.coordinates, element));
      ...
      if (topLeft[1] === bottomRight[1] && topLeft[0] === bottomRight[0]){
        bounds_ = [[(topLeft[1] - 0.1).toFixed(2) ,(topLeft[0] - 0.1).toFixed(2)], [(bottomRight[1] + 0.1).toFixed(2), (bottomRight[0] + 0.1).toFixed(2)]];
      }
      else{
        bounds_ = [[topLeft[1].toFixed(2),topLeft[0].toFixed(2)], [bottomRight[1].toFixed(2),bottomRight[0].toFixed(2)]];
      }
      
      //let bounds_ = [[topLeft[1].toFixed(2) + 0.04 ,topLeft[0].toFixed(2)] + 0.04, [bottomRight[1].toFixed(2) - 0.04, bottomRight[0].toFixed(2) - 0.04]];

      console.log("Bordes ", bounds_);
      bootleaf.map.flyToBounds(bounds_);
    },
